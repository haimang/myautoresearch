...
2. **完成 run-scoped output workspace**
   ...
   - 对 `fx_spot` campaign，默认建立：
     ...
     output/fx_spot/<fx_run_id>/
       ...
       runs/
     ```
   - 如果用户不传 `--db`，`fx_spot` sweep 会把 DB 放到：
     ```text
     output/fx_spot/<fx_run_id>/tracker.db
     ```
   - `domains/fx_spot/train.py` 支持接收：
     - `--run-id`
     - `--artifact-root`
...
6. **完成 constraint activation**
   - `domains/fx_spot/search_space.json` 新增：
     - `sell_amount_mode = surplus_fraction | explicit_ratio | floor_probe`
     - `sell_amount_ratio`
     - `floor_buffer_target`
   - `domains/fx_spot/route_eval.py` 新增：
...
7. **完成 route semantics 修复**
   - `domains/fx_spot/quote_graph.py` 新增：
     - `route_signature`
     - `route_family`
     - degenerate bridge rejection 支撑
   - `framework/sweep.py` 对 `fx_spot` dynamic axis 做 degenerate route 过滤：
     ```text
     bridge_currency == sell_currency
     bridge_currency == buy_currency
     ```
...
8. **完成 uplift-capable mock pricing surface**
   - `domains/fx_spot/scenario_presets.py` 新增：
     - `uplift_corridor`
     - `constraint_stress`
   - `domains/fx_spot/mock_provider.py` 新增：
     - `pair_rate_edge_bps`
     - `amount_tiers`
...
10. **补充测试**
    - 新增：
      ```text
      tests/test_fx_v23_workspace.py
      ```
    - 修改：
      ```text
      tests/test_fx_spot_domain.py
      ```
    - 覆盖：
...
2. **修改文件**
   ...
   - `framework/sweep.py`
     - 新增 `--run-id` / `--output-root`，fx_spot workspace 建立，degenerate route 过滤，auto Pareto 输出到 run workspace。
...
```bash
uv run python domains/fx_spot/bayesian_refine.py \
...
```text
output/fx_spot/v23-bo-multiscenario-final/
```
...
1. **证据组织**
   - 从 `output/` 根目录平铺，升级为 `output/fx_spot/<fx_run_id>/` workspace。
...
> **我们已经把 `fx_spot` 从 v22 的多场景 Pareto observation，推进到 v23 的 run-scoped、constraint-active、human-readable、Bayesian-style frontier refinement 原型。**
