...
> **把 `spot_trader` 从“能批量生成 Pareto front 证据的多场景 mock domain”推进到“能按 run id 归档实验、输出人类可读 front、让约束真正塑形、并用贝叶斯式主动补点去逼近 frontier / knee”的研究系统。**
...
1. 为每次 `spot_trader` 执行建立 **run-scoped output workspace**。
...
2. `domains/spot_trader/train.py` 支持接收并沿用该 `--run-id`。
3. 若不显式传入，则自动生成，但必须：
   - 落到 manifest
   - 出现在 log header
...
1. 任意一次 `spot_trader` 执行都有唯一 `fx_run_id`。
...
1. 所有 plot 必须写到 `output/spot_trader/<fx_run_id>/campaigns/<campaign_id>/pareto/`
...
```text
output/spot_trader/<fx_run_id>/benchmarks/
```
...
3. v22 的 `spot_trader` smoke chain 不被破坏。
...
1. 每次 `spot_trader` 执行都有 `fx_run_id`，并写入独立 output workspace。
...
## 15. v23 完成回填 — run-scoped / readable Pareto / constrained BO 收口报告

### 15.1 工作日志

1. **建立 v23 todo-list 并按 Phase 执行**
   - 新增并完成：
     - `v23-run-workspace`
     - `v23-plot-readable`
     - `v23-constraint-active`
     - `v23-route-pricing`
     - `v23-bayesian-loop`
     - `v23-backtest-report`
   - 执行顺序与本文件计划一致：先证据目录，再可视化，再约束，再 route/pricing，最后做 Bayesian replay benchmark。

2. **完成 run-scoped output workspace**
   - `framework/sweep.py` 新增：
     - `--run-id`
     - `--output-root`
   - 对 `spot_trader` campaign，默认建立：
     ```text
     output/spot_trader/<fx_run_id>/
       manifest.json
       tracker.db
       ...
       runs/
     ```
   - 如果用户不传 `--db`，`spot_trader` sweep 会把 DB 放到：
     ```text
     output/spot_trader/<fx_run_id>/tracker.db
     ```
   - `domains/spot_trader/train.py` 支持接收：
     - `--run-id`
     - `--artifact-root`
   - 单个 candidate run 的 `result.json` 会写入 run workspace，而不是散落在 `output/` 根目录。

3. **完成 DB schema 的 v23 workspace 支撑**
   - `framework/core/db.py` 新增：
     ```sql
     experiment_runs
     ```
   - 记录：
     - filesystem-level experiment run id
     - domain
     - output_root
   ...
   - `runs` 新增：
     ...
     artifact_dir
     ```
   - `frontier_snapshots` 新增：
     ```text
     artifact_dir
     ```
   - 注意：这里的 `experiment_runs.id / fx_run_id` 是文件系统层的一次实验容器，仍然区别于 DB 内单个 candidate 的 `runs.id`。

4. **完成 Pareto plot 可读性止血**
   - `framework/pareto_plot.py` 改为默认不标注 dominated points。
   - front points 默认只标少数关键点：
     - knee
     - x 轴最小点
     - y 轴最大点
     - preservation 最优点
     - validity 最优点
     - top N
   - annotation 改用短标签，不再把完整 `sweep_tag` 直接贴到 PNG 上。
   - 新增：
     - overview plot
     - front-only plot
     - knee zoom plot
     ...
     - front CSV
     - front Markdown
   - 2D frontier 增加 smoothed frontier overlay，用多项式拟合在静态图上给出更连续的前沿曲线参考。

5. **完成 analyze.py 的可读 Pareto artifact 输出**
   - `framework/analyze.py --pareto --plot` 在 `run_metrics` / objective profile 路径下改用新的 artifact exporter。
   - 一次 analyze 会同时输出：
     ```text
     analyze_overview.png
     analyze_overview_front_only.png
     analyze_overview_knee_zoom.png
     analyze_overview_front.json
     analyze_overview_front.csv
     analyze_overview_front.md
     ```
   - 这解决了 v22 中“只有一张不可读 PNG”的问题。

6. **完成 constraint activation**
   - `domains/spot_trader/manifest/search_space.json` 新增：
     - `sell_amount_mode = surplus_fraction | explicit_ratio | floor_probe`
     - `sell_amount_ratio`
     - `floor_buffer_target`
   - `domains/spot_trader/route_eval.py` 新增：
     - aggressive explicit ratio
     - floor-probe sell amount
     - liquidity breach count
     - insufficient balance count
     - expired quote count
     - breach margin ratio
     - sell amount ratio
   - 现在 smoke / replay 中可以真实产生 infeasible candidates，不再是 v22 的 `infeasible = 0`。

7. **完成 route semantics 修复**
   - `domains/spot_trader/quote_graph.py` 新增：
     - `route_signature`
     - `route_family`
     - degenerate bridge rejection 支撑
   - `framework/sweep.py` 对 `spot_trader` dynamic axis 做 degenerate route 过滤：
     ```text
     bridge_currency == sell_currency
     bridge_currency == buy_currency
     ```
   - 这避免了“名义 via_xxx、实际 direct”的候选污染 route analysis。

8. **完成 uplift-capable mock pricing surface**
   - `domains/spot_trader/scenario_presets.py` 新增：
     - `uplift_corridor`
     - `constraint_stress`
   - `domains/spot_trader/mock_provider.py` 新增：
     - `pair_rate_edge_bps`
     - `amount_tiers`
   - mock quote 不再只能表达“略差于 mid 的 spread”，现在可以表达：
     - corridor price edge
     - amount bucket 改善
     - bridge route 的局部优势
   - 因此 v23 benchmark 中已经出现正向 `spot_uplift_bps > 0` 的 candidates。

9. **完成 Bayesian refinement / replay benchmark 工具**
   - 新增：
     ```text
     domains/spot_trader/bayesian_refine.py
     ```
   - 该工具执行：
     1. 从 search space 构造 finite candidate universe
     2. 过滤 no-op / non-held sell currency / degenerate bridge candidates
     3. 对候选全集做 mock replay，形成可比较的 true-front reference
     4. random baseline 采样
     5. Bayesian-style constrained acquisition 采样
     6. 写 DB / quote windows / quotes / route legs / run metrics
     7. 输出 benchmark summary
     8. 输出可读 Pareto artifact
   - Bayesian loop 使用：
     - bootstrapped ridge utility surrogate
     - bootstrapped ridge feasibility surrogate
     - posterior uncertainty
     - `p_feasible`
     - uplift corridor prior
     - route complexity / feasibility penalty
   - acquisition 不是更大的 Cartesian brute-force，而是：
     ```text
     seed observations
       -> fit feasibility surrogate
       -> fit utility surrogate
       -> score unseen candidates
       -> evaluate next batch
       -> repeat until quote budget used
     ```

10. **补充测试**
    - 新增：
      ```text
      tests/test_spot_trader_v23_workspace.py
      ```
    - 修改：
      ```text
      tests/test_spot_trader_domain.py
      ```
    - 覆盖：
      - `--run-id` 创建 run-scoped workspace
      - `manifest.json` / workspace DB 生成
      - Bayesian replay smoke 输出 benchmark summary
      - floor-probe 触发 infeasible evidence
      - degenerate bridge rejection
      - v23 扩展 metrics 写入
...
1. **新增文件**
   - `domains/spot_trader/bayesian_refine.py`
     - v23 constrained Bayesian-style replay / refinement / benchmark 工具。
   - `tests/test_spot_trader_v23_workspace.py`
     - v23 workspace 与 Bayesian smoke 测试。

2. **修改文件**
   - `framework/core/db.py`
     - 新增 `experiment_runs`、`experiment_run_id`、`artifact_dir` schema 与 `save_experiment_run()`。
   - `framework/sweep.py`
     - 新增 `--run-id` / `--output-root`，spot_trader workspace 建立，degenerate route 过滤，auto Pareto 输出到 run workspace。
   - `framework/pareto_plot.py`
     - 可读图层、短标签、front-only/knee/table artifact、smoothed frontier overlay。
   - `framework/analyze.py`
     - run_metrics Pareto plot 改用新的 multi-artifact exporter。
   - `domains/spot_trader/train.py`
     - 接收 run id / artifact root，写 per-run `result.json`。
   - `domains/spot_trader/route_eval.py`
     - 新增 sell amount modes、constraint metrics、route metadata。
   - `domains/spot_trader/quote_graph.py`
     - 新增 route signature/family 与 degenerate bridge rejection。
   - `domains/spot_trader/mock_provider.py`
     - 新增 pricing edge / amount tier 支撑。
   - `domains/spot_trader/scenario_presets.py`
     - 新增 `uplift_corridor` / `constraint_stress`。
   - `domains/spot_trader/manifest/search_space.json`
     - 新增 constraint-active amount axes 与新 quote scenarios。
   - `domains/spot_trader/metrics.py`
     - 新增 constraint / diagnostic metrics role。
   - `tests/test_spot_trader_domain.py`
     - 更新 v23 metric 预期并新增 constraint / degenerate route 测试。
...
uv run python -m pytest \
  tests/test_spot_trader_domain.py \
  tests/test_spot_trader_v23_workspace.py \
  tests/test_run_metrics_pareto.py \
  tests/test_sweep_dynamic.py \
  -q
...
### 15.4 v23 benchmark 结果

#### 15.4.1 主 benchmark：`v23-bo-multiscenario-final`

执行：

```bash
uv run python domains/spot_trader/bayesian_refine.py \
  --run-id v23-bo-multiscenario-final \
  --output-root output \
  --scenarios cn_exporter_core,usd_importer_mix,global_diversified,asia_procurement_hub \
  --budget 140 \
  --seed-observations 20 \
  --batch-size 10 \
  --max-universe 2600 \
  --seed 45
```
...
```text
output/spot_trader/v23-bo-multiscenario-final/
```
...
| fx_route_legs | 457 |
| infeasible_runs | 34 |
| positive_uplift_runs | 7 |
| frontier_snapshots | 2 |

strategy 对比：

| strategy | budget | front points | true front hits | true front recall | best uplift bps | best preservation | infeasible rate |
|---|---:|---:|---:|---:|---:|---:|---:|
| random | 140 | 34 | 6 | 0.0526 | 0.4276 | 1.00004276 | 0.2000 |
| bayesian | 140 | 34 | 11 | 0.0965 | 0.3944 | 1.00003944 | 0.0429 |

结论：

1. Bayesian loop 在同样 140 quote budget 下，true-front hit 数从 6 提升到 11。
2. true-front recall 从 5.26% 提升到 9.65%。
3. infeasible rate 从 20.00% 降到 4.29%。
4. random 在这次 seed 下找到略高的 best uplift，但 Bayesian 也找到正向 uplift，并且用更低约束代价逼近 front。
5. 这说明当前 Bayesian loop 的价值主要体现在：
   - 更好的 front coverage
   - 更低的 infeasible waste
   - 更稳定地避开 constraint breach
6. 但它还没有在所有 seed / budget 下稳定压过 random 的单点 best uplift。

#### 15.4.2 扩展预算 benchmark：`v23-bo-multiscenario-extended`

执行：

```bash
uv run python domains/spot_trader/bayesian_refine.py \
  --run-id v23-bo-multiscenario-extended \
  --output-root output \
  --scenarios cn_exporter_core,usd_importer_mix,global_diversified,asia_procurement_hub \
  --budget 180 \
...
| strategy | budget | front points | true front hits | true front recall | best uplift bps | best preservation | infeasible rate |
|---|---:|---:|---:|---:|---:|---:|---:|
| random | 180 | 35 | 8 | 0.0734 | 0.1191 | 1.00001191 | 0.1944 |
| bayesian | 180 | 38 | 6 | 0.0550 | 0.3202 | 1.00003202 | 0.0444 |

结论：

1. 扩展预算下，Bayesian 找到了更高的 best uplift。
2. Bayesian 的 infeasible rate 仍然显著低于 random。
3. 但 true-front recall 在该 seed 下低于 random，说明当前 acquisition 的 front coverage 对初始 seed 与候选分布仍敏感。
4. 因此 v23 已经实现“真实 Bayesian-style constrained refinement loop”，但 acquisition 仍需要 v23.x/v24 继续校准。

### 15.5 Pareto 可视化输出

主 benchmark 中，每个 strategy campaign 已输出：

```text
output/spot_trader/v23-bo-multiscenario-final/
  campaigns/
    v23-bayesian-v23-bo-multiscenario-final/
      pareto/
        overview.png
        overview_front_only.png
        overview_knee_zoom.png
        overview_front.json
        overview_front.csv
        overview_front.md
        analyze_overview.png
        ...
        analyze_overview_front.csv
        analyze_overview_front.md
    v23-random-v23-bo-multiscenario-final/
      pareto/
        ...
```
...
1. dominated 点不再贴满文字。
2. front 点只标关键点。
3. 静态 PNG 上有 smoothed frontier overlay。
4. 完整细节进入 CSV / Markdown / JSON。
5. 每张图可以从路径反推出：
   - `fx_run_id`
   - strategy campaign
   - Pareto artifact 类型

这意味着 v23 已经把 v22 的“乱码 PNG”改造成了可复核的图 + 表组合。

### 15.6 对“光滑前沿曲线”的实现说明

v23 当前实现的 smooth frontier 是：

1. 对 observed Pareto front 按 x 轴排序。
2. 对相同 x 的点取该 x 下更优的 y。
3. 使用低阶多项式拟合生成 200 个插值点。
4. 在 overview / front-only / knee plot 中叠加 smoothed frontier line。

这不是在声称真实 FX market frontier 本身是光滑函数，而是在静态图中提供：

> **对 observed non-dominated points 的连续视觉近似，帮助人类读取 trade-off 曲线形状。**

换句话说，v23 得到的是：

```text
observed Pareto points
  + Bayesian-style active sampling
  + smoothed visual approximation
```
...
v23 相比 v22 已完成 5 个实质跃迁：

1. **证据组织**
   - 从 `output/` 根目录平铺，升级为 `output/spot_trader/<fx_run_id>/` workspace。

2. **可视化**
   - 从不可读 PNG，升级为 overview / front-only / knee / table artifacts。

3. **约束**
   - 从 `infeasible = 0` 的伪安全状态，升级为可以真实产生并统计 constraint breach。

4. **price surface**
   - 从单纯 spread loss，升级为可表达 corridor / amount-tier / positive uplift 的 mock surface。

5. **搜索机制**
   - 从 Cartesian sweep + post-hoc Pareto，升级为 constrained Bayesian-style replay refinement。

仍然存在的 v23.x 问题：

1. acquisition 对 seed 和候选分布仍敏感。
2. Bayesian loop 已经能降低 infeasible waste，但不总是稳定最大化单点 uplift。
3. smooth frontier 目前是 visual approximation，不是 surrogate posterior frontier 的显式置信带。
4. benchmark 仍基于 deterministic local mock provider，没有接 Airwallex sandbox quote。
5. route split / parallel execution 仍只是通过 amount/route candidate 近似，没有完整 portfolio-level multi-route optimizer。

### 15.8 v23 收口结论

v23 可以收口为：

> **我们已经把 `spot_trader` 从 v22 的多场景 Pareto observation，推进到 v23 的 run-scoped、constraint-active、human-readable、Bayesian-style frontier refinement 原型。**

更严格地说：

1. v23 已经真实使用 surrogate + posterior uncertainty + acquisition 做主动补点。
2. v23 已经可以在固定 quote budget 下与 random baseline 对比。
3. v23 已经生成可读的 front-only / knee / smoothed frontier 图。
4. v23 已经能让 liquidity constraint 真实影响实验结果。
5. v23 已经能在 mock quote surface 中发现正向 uplift。

因此，v23 的完成状态不是“最终 FX 决策系统”，而是：

> **一个可复现、可回放、可视化可读、并具备 constrained Bayesian refinement 能力的 FX autoresearch 实验底座。**

下一阶段如果继续推进，最值得做的是：

1. 把 surrogate 从 utility scalar 扩展到真正的 multi-objective posterior。
2. 增加 uncertainty band / confidence region plot。
3. 加入 split-route / portfolio optimizer。
4. 接入 Airwallex sandbox quote-only adapter，并保持 live conversion disabled。
