...
Train a Gomoku (五子棋) AI using the autoresearch experiment loop on Apple Silicon (MLX). The agent autonomously modifies `domains/gomoku/train.py` to discover the best neural network architecture and training strategy for beating increasingly strong opponents.
...
│           ├── web_app.py   # FastAPI backend                           [READ-ONLY]
│           ├── index.html
│           ├── app.js
│           └── styles.css
├── docs/
│   ├── program.md           # Framework template (domain-agnostic)
│   └── caveats.md           # Known pitfalls and troubleshooting
├── domains/
│   └── gomoku/
│       ├── program.md       # THIS FILE — Gomoku domain agent instructions
├── output/                  # All generated artifacts (gitignored)
│   └── tracker.db           # SQLite database — legacy research context
├── updates/                 # Design documents (gitignored)
├── pyproject.toml
└── uv.lock
```
...
**`domains/gomoku/train.py` is the ONLY file you modify.** All other source files are read-only.
...
1. Read the in-scope files:
   - `domains/gomoku/game.py` — board engine (BOARD_SIZE=15, WIN_LENGTH=5), BatchBoards for parallel self-play
...
### What you CAN do
- Modify `domains/gomoku/train.py` — everything inside is fair game: model architecture, optimizer, hyperparameters, self-play strategy, batch size, temperature schedule, replay buffer, MCTS, loss function, etc.

### What you CANNOT do
- Modify any file other than `domains/gomoku/train.py`
- Install new packages or add dependencies
- Modify the evaluation harness (`evaluate_win_rate` in `domains/gomoku/prepare.py`)
...
```bash
uv run python domains/gomoku/train.py --time-budget 300 --eval-level 0 --probe-games 50 --eval-interval 15 --full-eval-games 200
```
